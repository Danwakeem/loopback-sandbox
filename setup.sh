...
printf "\n${RED}>> Creating database in Cloudant${PLAIN}"		
curl --request PUT --url http://$USER:$PASSWORD@$HOST:$PORT/$DATABASE/sample_doc -d '{"keyword": "abc","loopback__model__name": "MyModel"}' > /dev/null 2>&1		
DB_OUTPUT=$?		
if [ "$DB_OUTPUT" -ne 0 ]; then		
    printf "\n\n${CYAN}Status: ${PLAIN}${RED}Sample doc could not be created. Terminating setup.${PLAIN}\n\n"		
    exit 1		
fi		
printf "\n${CYAN}Sample doc created succesfully.${PLAIN}\n"	
